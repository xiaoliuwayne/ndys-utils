...
function tryCatchBlock(fn) {
  ...
  return function (...args) {
    try {
      ...
      console.error(`tryCatchBlock:try function:\n ${fn} \nerror, ErrorName: ${errName}`);
      return errName;
    }
  };
}
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (isObj) {
          ...
          console.error(`promiseCatchBlock:try function:\n ${fn} \nerror, ErrorName: ${errName}`);
        }
      } catch (err) {
...
function convertToBoolean(val, falseLabels = []) {
  const finalFalseLabels = [null, undefined, 0, "", "false", false, ...falseLabels]; // '0'
...
function getRepetedEleIndexesInArr(arr, isUniqueObj = false) {
  ...
  return Array.from(allIndexes).filter((el, index) => !uniqueArrIndexes.includes(el));
}
/**
 * @author WayneLiu
 * @function
...
function generateUniqueEleArr(arr, isUniqueObj = false) {
  ...
  if (isArr) {
    ...
    const uniqueArrIndexes = getUniqueArrIndexes(arr, isUniqueObj);
    // 根据索引数组生产新的数组
    return copy.filter((element, index, array) => uniqueArrIndexes.includes(index));
  } else {
